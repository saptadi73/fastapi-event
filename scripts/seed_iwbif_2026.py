...
async def seed():
    async with AsyncSessionFactory() as db:
        ...
        for idx, (email, full_name, company, job, country, sector, package_code, looking, preferred, interests, needs) in enumerate(DELEGATES, 1):
            ...
            await ensure(db, Payment, order_id=order.id, defaults=dict(provider="midtrans", provider_transaction_id=f"MT-IWBIF26-{idx:04d}", provider_order_id=order.order_number, payment_type="credit_card", gross_amount=order.total_amount, currency="USD", transaction_status=PaymentStatus.SUCCESS, fraud_status="accept", raw_response='{"seed":true}', paid_at=at(3 + idx), expired_at=None))
