...
async def seed():
    async with AsyncSessionFactory() as db:
        ...
        for code, name, amount in PACKAGES:
            package[code] = await ensure(db, DelegatePackage, event_id=event.id, code=code, defaults=dict(name=name, currency="USD", amount=amount, is_active=True))
        activity = {}
        ...
        for idx, (email, full_name, company, job, country, sector, package_code, looking, preferred, interests, needs) in enumerate(DELEGATES, 1):
            ...
            order = await ensure(db, Order, registration_id=reg.id, order_number=f"ORD-IWBIF26-{idx:04d}", defaults=dict(subtotal=package[package_code].amount, discount_amount=0, tax_amount=0, service_fee=0, total_amount=package[package_code].amount, currency="USD", status=OrderStatus.PAID, expires_at=at(10)))
            seeded_payment = await ensure(db, Payment, order_id=order.id, defaults=dict(provider="doku", provider_transaction_id=f"DOKU-REQ-IWBIF26-{idx:04d}", provider_order_id=order.order_number, payment_type="doku_checkout", gross_amount=order.total_amount, currency="USD", transaction_status=PaymentStatus.SUCCESS, fraud_status=None, raw_response='{"provider":"doku","seed":true}', paid_at=at(3 + idx), expired_at=None, checkout_url=f"https://sandbox.doku.com/checkout/{order.order_number}"))
            await ensure(db, PaymentWebhookEvent, provider="doku", request_id=f"DOKU-NOTIFY-IWBIF26-{idx:04d}", defaults=dict(payment_id=seeded_payment.id, event_status="SUCCESS", payload={"order":{"invoice_number":order.order_number,"amount":float(order.total_amount)},"transaction":{"status":"SUCCESS"}}))
