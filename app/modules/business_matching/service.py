...
class BusinessMatchingService:
    ...
    async def create_conversation(db, user_id, event_id, payload):
        ...
        mine = await Repo.profile(db, event_id, me.id)
        if not mine or not target or not target.available_for_matching or not target.allow_messages or await Repo.blocked(db, event_id, me.id, target.participant_id): raise HTTPException(403, "Messaging is not allowed")
    ...
    async def send_message(db, user_id, conversation_id, payload):
        ...
        msg = Message(conversation_id=conversation_id, sender_participant_id=me.id, body=payload.body, reply_to_message_id=payload.reply_to_message_id)
    ...
    async def create_meeting(db, user_id, event_id, payload):
        ...
        await BusinessMatchingService._event(db, meeting, user_id, "meeting_request", "Permintaan meeting", MessageType.MEETING_REQUEST)
    ...
    async def transition(db, user_id, meeting_id, command, confirm=None):
        ...
        await BusinessMatchingService._event(db, meeting, user_id, f"meeting_{meeting.status.value}", f"Meeting {meeting.status.value}", MessageType.SYSTEM)
        await db.commit(); await db.refresh(meeting); return meeting
    ...
    async def _event(db, meeting, actor_user_id, event_type, title, message_type):
        ...
        if meeting.conversation_id: db.add(Message(conversation_id=meeting.conversation_id, sender_participant_id=meeting.requester_participant_id, message_type=message_type, body=title, meeting_id=meeting.id))
