...
async def seed():
    ...
    async with AsyncSessionFactory() as db:
        ...
        # Seed canonical English and Simplified Chinese email copy. Rerunning
        # remains idempotent while templates stay editable by organizers.
        for locale, templates in DEFAULT_TEMPLATES_BY_LOCALE.items():
            for trigger, (subject, body) in templates.items():
                await ensure(db, EmailNotificationTemplate, event_id=event.id, trigger=trigger, locale=locale, defaults=dict(subject_template=subject, body_template=body, available_variables=TRIGGER_VARIABLES[trigger], is_enabled=True))
        ...
        exhibitor_package = await ensure(db, DelegatePackage, event_id=event.id, code="EXHIBITOR", defaults=dict(name="Exhibitor Package - USD200", package_type="exhibitor", selection_mode="optional", description="Exhibitor access without requiring delegate registration", display_order=20, currency="USD", amount=Decimal("200"), payment_amount_idr=to_idr(Decimal("200")), is_active=True))
        packages_by_code["EXHIBITOR"] = exhibitor_package
        rate_specs = {"A": [("sharing", "Twin Sharing Basis", Decimal("500"), to_idr(Decimal("500")), True), ("single", "Single Room", Decimal("700"), to_idr(Decimal("700")), False)], "B": [("sharing", "Twin Sharing Basis", Decimal("400"), to_idr(Decimal("400")), True), ("single", "Single Room", Decimal("550"), to_idr(Decimal("550")), False)], "TRIP_BANDUNG": [("sharing", "Twin Sharing Basis", Decimal("200"), to_idr(Decimal("200")), True), ("single", "Single Room", Decimal("300"), to_idr(Decimal("300")), False)], "EXHIBITOR": [("standard", "Exhibitor Access", Decimal("200"), to_idr(Decimal("200")), True)]}
